...
static void pmw3610_async_init(struct k_work *work) {
    ...
    LOG_INF("PMW3610 async init attempt %d step %d", data->init_attempt + 1,
            data->async_init_step);

    data->err = async_init_fn[data->async_init_step](dev);
    if (data->err) {
        data->init_attempt++;
        LOG_ERR("PMW3610 initialization failed in attempt %d step %d: %d",
                data->init_attempt, data->async_init_step, data->err);
